...
document.addEventListener('DOMContentLoaded', () => {
    ...
    console.log('🎯 Live Ticker & Yields: ENABLED');
    ...
    // Initialize ticker and yields immediately
    console.log('📊 Initializing live ticker...');
    updateLiveTicker();
    setInterval(updateLiveTicker, 2000); // Update every 2 seconds
    
    console.log('📈 Initializing US yields...');
    updateUSYields();
    setInterval(updateUSYields, 10000); // Update every 10 seconds
...
// ===== LIVE TICKER - REAL DATA ONLY =====

async function updateLiveTicker() {
    const ticker = document.getElementById('liveTicker');
    
    try {
        console.log('🔄 Fetching REAL live ticker data...');
        
        // Use working CORS proxy
        const proxy = 'https://api.codetabs.com/v1/proxy?quest=';
        
        // Fetch real forex data
        const forexUrl = `https://financialmodelingprep.com/api/v3/fx/EURUSD,GBPUSD,USDJPY,AUDUSD,USDCAD,USDCHF?apikey=${CONFIG.FMP_API_KEY}`;
        const quotesUrl = `https://financialmodelingprep.com/api/v3/quote/XAUUSD,XAGUSD,BTCUSD,DX-Y.NYB?apikey=${CONFIG.FMP_API_KEY}`;
        
        const [forexRes, quotesRes] = await Promise.all([
            fetch(proxy + encodeURIComponent(forexUrl)),
            fetch(proxy + encodeURIComponent(quotesUrl))
        ]);
        ...
        if (Array.isArray(forexData)) {
            forexData.forEach(q => {
                const price = q.ask || q.bid || q.price || 0;
                const change = (q.changes || 0) * 100;
                const isPos = change > 0;
                
                html += `
                    <div class="ticker-item ${isPos ? 'positive' : 'negative'}">
                        <span class="ticker-symbol">${q.ticker}</span>
                        <span class="ticker-price">${price.toFixed(5)}</span>
                        <span class="ticker-change">${isPos ? '+' : ''}${change.toFixed(2)}%</span>
                    </div>
                `;
            });
        }
        ...
        if (Array.isArray(quotesData)) {
            quotesData.forEach(q => {
                ...
                if (q.symbol === 'XAUUSD') { name = 'Gold'; decimals = 2; }
                else if (q.symbol === 'XAGUSD') { name = 'Silver'; decimals = 3; }
                else if (q.symbol === 'BTCUSD') { name = 'Bitcoin'; decimals = 0; }
                else if (q.symbol === 'DX-Y.NYB') { name = 'DXY'; decimals = 3; }
                ...
                html += `
                    <div class="ticker-item ${isPos ? 'positive' : 'negative'}">
                        ...
                        <span class="ticker-price">${(q.price || 0).toFixed(decimals)}</span>
                        <span class="ticker-change">${isPos ? '+' : ''}${change.toFixed(2)}%</span>
                    </div>
                `;
            });
        ...
        if (html) {
            ticker.innerHTML = html + html; // Duplicate for scrolling
            console.log('✅ LIVE ticker data loaded successfully');
        } else {
        ...
        ticker.innerHTML = '<div class="ticker-loading">⚠️ Connecting to live data feed...</div>';
        
        // Retry after 5 seconds
        setTimeout(updateLiveTicker, 5000);
    }
}

async function updateUSYields() {
    const yieldsEl = document.getElementById('yieldsData');
    
    try {
        console.log('🔄 Fetching REAL yields data...');
        
        const proxy = 'https://api.codetabs.com/v1/proxy?quest=';
        const url = `https://financialmodelingprep.com/api/v3/quote/^TNX,^FVX,^IRX?apikey=${CONFIG.FMP_API_KEY}`;
        ...
        const html = data.map(q => {
            let period = '';
            if (q.symbol === '^TNX') period = '10Y';
            else if (q.symbol === '^FVX') period = '5Y';
            else if (q.symbol === '^IRX') period = '3M';
            
            const rate = q.price || 0;
        ...
        yieldsEl.innerHTML = html;
        console.log('✅ LIVE yields data loaded successfully');
        
    } catch (error) {
        console.error('❌ Yields fetch failed:', error);
        yieldsEl.innerHTML = '<div class="yield-loading">⚠️ Connecting to yields feed...</div>';
        
        // Retry after 5 seconds
        setTimeout(updateUSYields, 5000);
    }
}
...
// Fetch REAL macro data using CORS proxy
async function fetchMacroData() {
    try {
        console.log('🔄 Fetching REAL macro data (DXY, Yields, VIX, Oil)...');
        
        const proxy = 'https://api.codetabs.com/v1/proxy?quest=';
        const symbols = 'DX-Y.NYB,^TNX,^VIX,CL=F';
        const url = `https://financialmodelingprep.com/api/v3/quote/${symbols}?apikey=${CONFIG.FMP_API_KEY}`;
        ...
        console.log('✅ REAL macro data loaded:', {
            DXY: macroData.dxy.price,
            Yields: macroData.yields.price,
            VIX: macroData.vix.price,
            Oil: macroData.oil.price
        });
        ...
        return fetchMacroData(); // Recursive retry
    }
}
